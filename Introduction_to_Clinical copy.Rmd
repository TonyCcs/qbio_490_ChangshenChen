...
```{r setup}
    knitr::opts_knit$set(root.dir = normalizePath("/Users/chenchangshen/Desktop/FABIO490/analysis_data")) 
```
...
mtcars_copy <- mtcars[, -which(colnames(mtcars) == "am")]
```
...
```{r}
write.csv(clinic, "/Users/chenchangshen/Desktop/FAQBIO490/490_cluster/analysis_data/brca_clinical_data.csv", row.names = FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/Users/chenchangshen/Desktop/FAQBIO490/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
age_5th_patient <- clinic[clinic$Tumor_Sample_Barcode == "TCGA-4H-AAAK", "age_at_diagnosis"]

age_at_diagnosis <- clinic[clinic$Tumor_Sample_Barcode == "TCGA-3C-AAAU", "age_at_diagnosis"]

days_to_last_followup <- clinic[clinic$Tumor_Sample_Barcode == "TCGA-3C-AAAU", "last_contact_days_to"]

age_at_diagnosis + (as.numeric(days_to_last_followup) / 365)
...
plot(clinic$age_at_diagnosis, 
     ...
     main = "Scatter Plot of Age at Diagnosis vs. Death days", 
...
The p-value is 0.2, suggesting that there is no strong evidence to conclude that there is a statistically significant difference in survival rates among the different age groups.
