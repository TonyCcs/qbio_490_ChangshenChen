...
```{r Exercise 1.2}
dim(colData(rna_se))
dim(elementMetadata(rowRanges(rna_se)))
...
```{r Exercise 2.1}
# write code here
head(colData(rna_se)$age_at_index, 15)
```
...
```{r Exercise 2.5}
rna_clinical <- rna_clinical[, !(colnames(rna_clinical) %in% c("treatments", "primary_site", "disease_type"))]
```
...
```{r Exercise 3.1}
# write code here
head(elementMetadata(rowRanges(rna_se)))
...
```{r Exercise 3.2}
rna_genes <- as.data.frame(elementMetadata(rowRanges(rna_se)))
...
```{r Exercise 4.2}
valid_patients <- !is.na(colData(rna_se)$age_at_index)
rna_counts <- as.data.frame(assay(rna_se, "unstranded")[, valid_patients])
```

Fetch the data for the first 5 patients and the first 15 rows.

```{r Exercise 4.3}
rna_counts_subset <- rna_counts[1:15, 1:5]
```
...
rownames(rna_counts) <- colnames(rna_genes$gene_id)
```
...
```{r Exercise 5.10}
# write code here
tumor_samples_mask <- rna_clinical$definition != "Solid Tissue Normal"
```
...
```{r Exercise 5.11}
# write code here
rna_clinical <- rna_clinical[tumor_samples_mask, ]
rna_counts <- rna_counts[, tumor_samples_mask]
...
valid_vital_status <- !is.na(rna_clinical$vital_status) & rna_clinical$vital_status != "Not Reported"

rna_clinical <- rna_clinical[valid_vital_status, ]
rna_counts <- rna_counts[, valid_vital_status]
```
