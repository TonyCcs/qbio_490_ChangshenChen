...
```{r}
rna_clinical <- read.csv("/Users/chenchangshen/Desktop/FAQBIO490/490_cluster/analysis_data/brca_rna_clinical_data.csv")
rna_genes <- read.csv("/Users/chenchangshen/Desktop/FAQBIO490/490_cluster/analysis_data/brca_rna_gene_data.csv")
rna_counts <- read.csv("/Users/chenchangshen/Desktop/FAQBIO490/490_cluster/brca_rna_count_data.csv", row.names=1)
...
rna_clinical <- rna_clinical[!is.na(rna_clinical$age_at_index),]

clinical_samples <- rna_clinical$patient
counts_samples <- colnames(rna_counts)
counts_samples <- gsub("\\.", "-", counts_samples)
colnames(rna_counts) <- counts_samples
clinical_samples <- toupper(clinical_samples)
counts_samples <- toupper(counts_samples)
common_samples <- intersect(clinical_samples, counts_samples)
rna_clinical_sub <- rna_clinical[rna_clinical$patient %in% common_samples, ]
rna_clinical_sub <- rna_clinical_sub[match(common_samples, rna_clinical_sub$patient), ]
rna_counts_sub <- rna_counts[, common_samples]

gene_totals <- rowSums(rna_counts_sub)
expressed_genes <- gene_totals >= 1000
rna_counts_filtered <- rna_counts_sub[expressed_genes, ]

dim(rna_counts_filtered)
dim(rna_clinical_sub)


unique(rna_clinical_sub$definition)
```
...
results <- merge(results, rna_genes[, c("gene_id", "gene_name")], by = "gene_id", all.x = TRUE)
results$gene_name[is.na(results$gene_name)] <- results$gene_id[is.na(results$gene_name)]
...
top-right genes: highly significant and up-regulated
bottom-right genes: not significant and up-regulated
top-left genes: highly significant and down-regulated
bottom-left genes: not significant and down-regulated
top-middle genes: highly significant and minimal change
bottom-middle genes: not significant and minimal change
