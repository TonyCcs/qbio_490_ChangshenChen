...
rna_counts <- read.csv("/Users/chenchangshen/Desktop/FAQBIO490/490_cluster/analysis_data/brca_rna_count_data.csv", row.names = 1)
...
rna_clinical$age_scaled <- scale(rna_clinical$age_at_index)
rna_counts <- as.data.frame(lapply(rna_counts, as.numeric))
rna_counts<- rna_counts[!is.na(rna_genes$gene_id),]
gene_totals <- rowSums(rna_counts)

rna_counts <- rna_counts[gene_totals >= 1000, ]
rna_genes <- rna_genes[gene_totals >= 1000, ]
...
dds <- DESeqDataSetFromMatrix(countData = rna_counts,
                              ...
                              design=  ~age_at_index + paper_BRCA_Subtype_PAM50 + definition
)
